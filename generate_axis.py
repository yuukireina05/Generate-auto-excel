...
# 提取所有含auto刀作业的行
auto = df[df['Unnamed: 1'].str.contains('a|b|c', na=False,case=False)]
auto = auto[~auto['Unnamed: 1'].str.contains('at|bt|ct', na=False,case=False)]
auto = auto[~auto['Unnamed: 2'].str.contains('物防', na=False,case=False)]
auto2 = auto.copy()
...
# 判断x面作业
phase = None
if 'C' in auto2.iloc[0,0]:
    phase = 'C'
elif 'B' in auto2.iloc[0,0]:
    phase = 'B'
elif 'A' in auto2.iloc[0,0]:
    phase = 'A'

# 生成各个王作业的DataFrame
auto_1 = auto2[auto2['Unnamed: 1'].str.contains(phase+'1', na=False,case=False)].iloc[:, 1:8]
auto_2 = auto2[auto2['Unnamed: 1'].str.contains(phase+'2', na=False,case=False)].iloc[:, 1:8]
auto_3 = auto2[auto2['Unnamed: 1'].str.contains(phase+'3', na=False,case=False)].iloc[:, 1:8]
auto_4 = auto2[auto2['Unnamed: 1'].str.contains(phase+'4', na=False,case=False)].iloc[:, 1:8]
auto_5 = auto2[auto2['Unnamed: 1'].str.contains(phase+'5', na=False,case=False)].iloc[:, 1:8]
...
#保存文件
ew = StyleFrame.ExcelWriter('axis_test.xlsx')
auto_sf.to_excel(ew)
ew.save()
ew.close()